...
foreach ($categoryList as $category) {
    $content = getContent($category->nodeValue);
//    $content = getContent('https://www.vuivui.com/sitemap/sitemap_6597.xml');
//    vdd($content);
    if(preg_match('~sitemap_(\d+)\.xml$~', $category->nodeValue, $categoryIdMatch)){
        $categoryId = $categoryIdMatch[1];
//        vdd($categoryId);
    } else{
        echo 'Error1 : Can\'t get category id <a target="_blank" href="' . $category->nodeValue . '">' . $category->nodeValue . '</a><br>';
        flush();
    }
...
//                    die();
        ...
        echo 'Error 11: Can\'t get content <a target="_blank" href="' . $category->nodeValue . '">' . $category->nodeValue . '</a><br>';
        flush();
    }
}